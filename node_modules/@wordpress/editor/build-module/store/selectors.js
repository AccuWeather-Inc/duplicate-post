...
import { getFreeformContentHandlerName, getDefaultBlockName, isUnmodifiedDefaultBlock } from '@wordpress/blocks';
...
import { EDIT_MERGE_PROPERTIES, POST_UPDATE_TRANSACTION_ID, PERMALINK_POSTNAME_REGEX, ONE_MINUTE_IN_MS, AUTOSAVE_PROPERTIES } from './constants';
...
export var hasEditorUndo = createRegistrySelector(function (select) {
  return function () {
...
export var hasEditorRedo = createRegistrySelector(function (select) {
  return function () {
...
export var isEditedPostDirty = createRegistrySelector(function (select) {
...
export var hasNonPostEntityChanges = createRegistrySelector(function (select) {
  return function (state) {
    ...
    return some(dirtyEntityRecords, function (entityRecord) {
...
export var getCurrentPost = createRegistrySelector(function (select) {
  return function (state) {
    var postId = getCurrentPostId(state);
    var postType = getCurrentPostType(state);
...
export var getPostEdits = createRegistrySelector(function (select) {
  return function (state) {
    var postType = getCurrentPostType(state);
    var postId = getCurrentPostId(state);
...
export var getReferenceByDistinctEdits = createRegistrySelector(function (select) {
  return function ()
  /* state */
  {
...
var getNestedEditedPostProperty = function getNestedEditedPostProperty(state, attributeName) {
  ...
  return _objectSpread(_objectSpread({}, getCurrentPostAttribute(state, attributeName)), edits[attributeName]);
};
/**
 * Returns a single attribute of the post being edited, preferring the unsaved
...
export var getAutosaveAttribute = createRegistrySelector(function (select) {
  return function (state, attributeName) {
    ...
    var postType = getCurrentPostType(state);
    var postId = getCurrentPostId(state);
    var currentUserId = get(select('core').getCurrentUser(), ['id']);
...
export var isEditedPostAutosaveable = createRegistrySelector(function (select) {
  return function (state) {
    ...
    var currentUserId = get(select('core').getCurrentUser(), ['id']); // Disable reason - this line causes the side-effect of fetching the autosave
    ...
    return ['title', 'excerpt'].some(function (field) {
      return getPostRawValue(autosave[field]) !== getEditedPostAttribute(state, field);
    });
  };
});
/**
...
export var getAutosave = createRegistrySelector(function (select) {
  return function (state) {
...
export var hasAutosave = createRegistrySelector(function (select) {
  return function (state) {
...
export var isSavingPost = createRegistrySelector(function (select) {
  return function (state) {
    var postType = getCurrentPostType(state);
    var postId = getCurrentPostId(state);
...
export var didPostSaveRequestSucceed = createRegistrySelector(function (select) {
  return function (state) {
    var postType = getCurrentPostType(state);
    var postId = getCurrentPostId(state);
...
export var didPostSaveRequestFail = createRegistrySelector(function (select) {
  return function (state) {
    var postType = getCurrentPostType(state);
    var postId = getCurrentPostId(state);
    return !!select('core').getLastEntitySaveError('postType', postType, postId);
...
export var getEditedPostContent = createRegistrySelector(function (select) {
  return function (state) {
    var postId = getCurrentPostId(state);
    var postType = getCurrentPostType(state);
